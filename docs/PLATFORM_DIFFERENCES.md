...
| **Agent runtime (PRoot Ubuntu + toolchains)** | ✅ Full — `RuntimeInstaller` (resume/SHA-256/extract) + `SandboxManager` auto-switch to isolated shell; `RuntimeSetupView` installs Core + opt-in stacks | ❌ N/A — no process/shell APIs in browser; agent runs cloud-only, terminal hidden | ⚠️ Native-shell fallback — `RuntimeSetupView` shows an Android-only notice and disables Install (installer preflight also refuses); agent + terminal run in the host shell (cmd/PowerShell) with the same approval gates and secret redaction |
| **APK install (PackageInstaller, no ADB)** | ✅ `ApkInstallerService` + receiver + FileProvider (MIUI) fallback | ❌ N/A | ❌ N/A — "Install APK from device" button hidden on desktop (CubicApp Builder still exports ZIP + GitHub workflow) |
| **Setup recommendations (notifications/battery/runtime/keys)** | ✅ Full — onboarding page 4 + Settings → Recommended setup; battery via `com.cubiclm.app/power` channel | ⚠️ Notifications row only (browser Notification API); battery/runtime rows hidden | ⚠️ Runtime/key/model rows only; notifications + battery rows hidden (Windows notification stub, no battery API) |
